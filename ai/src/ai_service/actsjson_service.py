...
language_model = ChatOpenAI(model_name="gpt-3.5-turbo", callbacks=[tracer], temperature=0)
...
def check_title(title, prompt):
    ...
    response = language_model.invoke([SystemMessage(content=system_prompt)])
    return response.content

def get_valid_pdf_paths(dataFrame_titles, prompt):
    array = []
    for _, row in dataFrame_titles.iterrows():
