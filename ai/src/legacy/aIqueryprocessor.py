...
def create_key_words_from_querry(keywords,prompt):
    matches = []
    for batch in chunk_list(keywords, 50):
...
def create_keywords_from_prompt(prompt, keywords):
    new_keywords=create_key_words_from_querry(keywords,prompt)
    evkeywords=evaluate_keywords_against_query(new_keywords,prompt)
    return evkeywords

np_key=np.array(df_key[0])
print(create_keywords_from_prompt("Czy można palić papierosy w kościele",np_key))
